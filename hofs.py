...
# Higher-order functions allow us to build new functions by passing in 
# other functions as inputs. This allows our code to be more versatile.
...
@typechecked
def foldl(f: Callable, z, L: List):
    if (L == []):
        return z
    return foldl(f, f(L[0], z), L[1::])

@typechecked
def foldr(f: Callable, z, L: List):
    if (L == []):
        return z
    return f(L[0], foldr(f, z, L[1::]))

@typechecked
def scan(f: Callable, z, L: List):
    if (L == []):
        return []
